param(
  [string]$InstallDir = $(if ($env:RAYCLAW_INSTALL_DIR) { $env:RAYCLAW_INSTALL_DIR } else { Join-Path $env:USERPROFILE '.local\bin' }),
  [switch]$CleanPath
)
...
function Resolve-Targets([string]$installDir, [string]$binName) {
  ...
  $cmd = Get-Command rayclaw -ErrorAction SilentlyContinue
  if ($cmd -and -not [string]::IsNullOrWhiteSpace($cmd.Source)) {
    $targets.Add($cmd.Source)
  }

  $targets.Add((Join-Path $env:USERPROFILE '.local\bin\rayclaw.exe'))
...
Write-Info "Uninstalling rayclaw..."
...
if ($removed -eq 0) {
  Write-Info "rayclaw binary not found. Nothing to uninstall."
  exit 0
}

Write-Info ""
Write-Info "rayclaw has been removed."
Write-Info "Optional cleanup (not removed automatically):"
Write-Info "  Remove-Item -Recurse -Force .\\rayclaw.data\\runtime"
Write-Info "  Remove-Item -Force .\\rayclaw.config.yaml,.\\rayclaw.config.yml"
