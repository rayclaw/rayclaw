# RayClaw Testing Guide

This document describes how to test every feature of RayClaw. It includes both automated tests (unit/integration) and manual black-box functional tests organized by user stories.
...
Since RayClaw is a multi-platform bot with external dependencies (LLM APIs, Telegram/Discord/WhatsApp APIs, DuckDuckGo), many features require live interaction testing.

### Prerequisites

1. A working `rayclaw.config.yaml` file with valid credentials
...
|---|-----------|-------|----------|
| 1.1 | First launch without config | Run `rayclaw start` with no config.yaml present | Auto-launches setup wizard |
| 1.2 | Normal startup | Fill config.yaml, run `rayclaw start` | Logs: Database initialized / Memory manager initialized / Scheduler started |
...
| 1.5 | CLI help | `rayclaw help` | Full output: commands, features, config docs |
| 1.6 | CLI version | `rayclaw version` | Output: `rayclaw {VERSION}` |
| 1.7 | Unknown command | `rayclaw foobar` | "Unknown command: foobar" + help text |
| 1.8 | Setup wizard | `rayclaw setup` | TUI interactive guide, provider/model selection |
| 1.9 | RAYCLAW_CONFIG env var | `RAYCLAW_CONFIG=/tmp/test.yaml rayclaw start` | Loads config from specified path |
...
| # | User Story | Steps | Expected |
|---|-----------|-------|----------|
| 25.1 | Install service | `rayclaw gateway install` | macOS: LaunchAgent; Linux: systemd service |
| 25.2 | Start service | `rayclaw gateway start` | Service starts |
| 25.3 | Check status | `rayclaw gateway status` | Reports running state |
| 25.4 | View logs | `rayclaw gateway logs 50` | Shows last 50 log lines |
| 25.5 | Stop service | `rayclaw gateway stop` | Service stops |
| 25.6 | Uninstall service | `rayclaw gateway uninstall` | Service removed |
...
```sh
# Remove test files
rm -f /tmp/rayclaw_test.txt /tmp/todos.txt /tmp/test.txt /tmp/session_test.txt

# Cancel any remaining scheduled tasks via the bot, or:
sqlite3 rayclaw.data/runtime/rayclaw.db "UPDATE scheduled_tasks SET status='cancelled' WHERE status='active';"
```
