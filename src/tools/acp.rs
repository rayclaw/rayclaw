...
pub fn make_acp_tools(manager: Arc<AcpManager>) -> Vec<Box<dyn Tool>> {
    vec![
        ...
        Box::new(AcpListSessionsTool::new(manager)),
    ]
}
...
impl Tool for AcpPromptTool {
    ...
    async fn execute(&self, input: serde_json::Value) -> ToolResult {
        ...
        match self.manager.prompt(session_id, message, timeout_secs).await {
            Ok(result) => {
...
mod tests {
    ...
    fn test_tool_names_unique() {
        ...
        sorted.sort();
        sorted.dedup();
        assert_eq!(sorted.len(), 4, "Tool names must be unique");
    }
