...
impl SetupApp {
    ...
    /// Load existing config values from rayclaw.config.yaml/.yml.
    ...
    fn validate_local(&self) -> Result<(), RayClawError> {
        for field in &self.fields {
            if self.is_field_required(field) && field.value.trim().is_empty() {
                return Err(RayClawError::Config(format!("{} is required", field.key)));
            }
        }
        ...
        if provider.is_empty() {
            return Err(RayClawError::Config("LLM_PROVIDER is required".into()));
        }
        ...
        tz.parse::<chrono_tz::Tz>()
            .map_err(|_| RayClawError::Config(format!("Invalid TIMEZONE: {tz}")))?;

        let data_dir = self.field_value("DATA_DIR");
        let dir = if data_dir.is_empty() {
            "./rayclaw.data".to_string()
        } else {
            data_dir
        };
        ...
        if !memory_token_budget_raw.is_empty() {
            let memory_token_budget = memory_token_budget_raw.parse::<usize>().map_err(|_| {
                RayClawError::Config("MEMORY_TOKEN_BUDGET must be a positive integer".into())
            })?;
        ...
        if !embedding_dim_raw.is_empty() {
            let embedding_dim = embedding_dim_raw.parse::<usize>().map_err(|_| {
                RayClawError::Config("EMBEDDING_DIM must be a positive integer".into())
            })?;
            if embedding_dim == 0 {
                return Err(RayClawError::Config(
    ...
    fn validate_online(&self) -> Result<Vec<String>, RayClawError> {
        ...
        .join()
        .map_err(|_| RayClawError::Config("Validation thread panicked".into()))?
    }
    ...
    fn model_options(&self) -> Vec<String> {
        let provider = self.field_value("LLM_PROVIDER");
        if let Some(preset) = find_provider_preset(&provider) {
            let mut opts: Vec<String> = preset.models.iter().map(|m| (*m).to_string()).collect();
...
fn run_with_spinner<T, F>(
    ...
    label: &str,
    work: F,
) -> Result<T, RayClawError>
where
    T: Send + 'static,
    F: FnOnce() -> Result<T, RayClawError> + Send + 'static,
{
    let (tx, rx) = mpsc::channel::<Result<T, RayClawError>>();
    std::thread::spawn(move || {
        let _ = tx.send(work());
    });
...
fn try_save(terminal: &mut DefaultTerminal, app: &mut SetupApp) -> Result<(), RayClawError> {
    ...
    let backup = match run_with_spinner(
        terminal,
        app,
        "Saving (3/3): writing rayclaw.config.yaml",
        move || save_config_yaml(Path::new("rayclaw.config.yaml"), &values),
    ) {
        Ok(v) => v,
        Err(e) => {
            app.status = format!("Cannot save: {e}");
    ...
    app.status = "Saved rayclaw.config.yaml".into();
    app.completed = true;
    Ok(())
}

fn run_wizard(mut terminal: DefaultTerminal) -> Result<bool, RayClawError> {
...
pub fn run_setup_wizard() -> Result<bool, RayClawError> {
    enable_raw_mode()?;
...
mod tests {
    ...
    fn test_save_config_yaml_preserves_discord_token_without_enabled_channels() {
        let yaml_path = std::env::temp_dir().join(format!(
            "rayclaw_setup_discord_test_{}.yaml",
            Utc::now().timestamp_nanos_opt().unwrap_or_default()
        ));
